...
class VendorController extends Controller {
    ...
    public function actionCreate() {
        ...
        $this->page_title ='Vendor management';
        
        $model->scenario = 'create';

        if (isset($_POST['Users'],$_POST['UserAddress'])) {
        ...
        //                                   
                  ...
                  //  } else {
                    ...
                    //   Yii::app()->user->setFlash('error', 'please upload Vendor image!!!'); 
                       //return false;
              ...
              $addressModel->created      = date('Y-m-d H:i:s');
              ...
              if ($model->save() && $addressModel->save()) {
                  
                  $transaction->commit();
                  Yii::app()->user->setFlash('success', 'Vendor created successfully');
                  $this->redirect(array('vendor/index'));
              } else {
                  Yii::app()->user->setFlash('error', 'Error creating in vendor');
              }
...
//              }
    ...
    public function actionUpdate($id) {
        $model = $this->loadModel($id);
        $addressModel   = UserAddress::model()->findAll(array("condition"=>"user_id =  $id"));
        $addressModel   = $addressModel[0];
        ...
        if (isset($_POST['Users'],$_POST['UserAddress'])) {
            ...
            try {
              ...
              if ($model->save() && $addressModel->save()) {
                  ...
                  Yii::app()->user->setFlash('error', 'Error updating in vendor');
              }
...
//                                    
    ...
    public function actionActivate($id) {
        ...
        if ($model->save()) {
            Yii::app()->user->setFlash('success', 'Vendor activated successfully!');
            $this->redirect(array('vendor/index'));
        } else {
            $msg    = $model->getErrors();
    ...
    public function actionDeactivate($id) {
        ...
        $model->status = 1;
        if ($model->save()) {
            Yii::app()->user->setFlash('success', 'Vendor Deactivated successfully!');
            $this->redirect(array('vendor/index'));
        } else {
            $msg    = $model->getErrors();
            foreach ($msg as $message) {
     ...
     public function actionBanned($id) {
        ...
        $model->status = 0;
        if ($model->save()) {
            Yii::app()->user->setFlash('success', 'Vendor Banned  successfully!');
            $this->redirect(array('vendor/index'));
        } else {
            $msg    = $model->getErrors();
            foreach ($msg as $message) {
    ...
    public function actioncustomerDelete($id) {
        ...
        try {
            $data->delete();
            Yii::app()->user->setFlash('success', "Vendor has been deleted successfully.");
        } catch (CDbException $e) {
            Yii::app()->user->setFlash('error', "Vendor could not be deleted");
        }
